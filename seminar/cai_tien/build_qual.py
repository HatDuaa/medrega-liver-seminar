...
NAVY="#1f3a5f"; RED="#ff3b30"; GREEN="#22c55e"; GREYT="#555"
...
for lang,L in TXT.items():
    ...
    ax[1].add_patch(Rectangle((GT[0],GT[1]),GT[2]-GT[0],GT[3]-GT[1],fill=False,edgecolor=GREEN,lw=1.4,ls="--"))
    ax[1].add_patch(Rectangle((PRED[0],PRED[1]),PRED[2]-PRED[0],PRED[3]-PRED[1],fill=False,edgecolor=RED,lw=2.2))
    inset(ax[1],[(GREEN,"--",1.2,GT),(RED,"-",2.0,PRED)])
    ...
    ax[1].plot([],[],color=RED,lw=2,label=L["lp"]); ax[1].plot([],[],color=GREEN,lw=1.4,ls="--",label=L["lg"])
